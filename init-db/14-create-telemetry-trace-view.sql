-- Gold layer: Telemetry Trace
-- Exposes high-frequency car telemetry samples aligned with GPS and lap context,
-- used for detailed telemetry overlays and on-track trace visualizations.
-- 
-- OPTIMIZATION NOTES:
-- - Uses LATERAL joins for efficient GPS timestamp matching
-- - Pre-filters sessions to reduce data volume
-- - Uses window functions for lap end calculation
-- - Indexes on source tables: car_telemetry(session_id, driver_id, date), car_gps(session_id, driver_id, date), laps(session_id, driver_id, lap_number)

-- Drop existing view if recreating
DROP MATERIALIZED VIEW IF EXISTS gold.telemetry_trace CASCADE;

CREATE MATERIALIZED VIEW gold.telemetry_trace AS
WITH laps_with_end AS (
...
telemetry_with_gps AS (
    ...
    -- Optimized: Uses LATERAL join with indexed timestamp range scan
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE cg.session_id = ct.session_id
          ...
          AND cg.date BETWEEN ct.date - INTERVAL '0.3 seconds' AND ct.date + INTERVAL '0.3 seconds'
...
telemetry_with_lap AS (
    ...
    -- Optimized: Uses indexed range scan on date_start/date_end
...
-- Create indexes on telemetry_trace for performance
CREATE INDEX IF NOT EXISTS idx_telemetry_trace_session_driver_timestamp 
    ON gold.telemetry_trace(session_id, driver_id, sample_timestamp);
CREATE INDEX IF NOT EXISTS idx_telemetry_trace_session_lap 
    ON gold.telemetry_trace(session_id, lap_number);
CREATE INDEX IF NOT EXISTS idx_telemetry_trace_season 
    ON gold.telemetry_trace(season);
CREATE INDEX IF NOT EXISTS idx_telemetry_trace_session_type 
    ON gold.telemetry_trace(session_type);
CREATE INDEX IF NOT EXISTS idx_telemetry_trace_lap_id 
    ON gold.telemetry_trace(lap_id) WHERE lap_id IS NOT NULL;
